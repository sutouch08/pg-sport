...
<table class="table table-striped">
	<thead>
    	<tr><th colspan="8" style="text-align:center;">รายงานยอดขาย แยกตามรายการสินค้า แสดงกำไรขั้นต้น</th></tr>
        <tr>
...
function report()
{
	...
	load_in();
	$.ajax({
		url:"report/reportController/saleReportController.php?sale_profit_item&report",
		type:"POST", cache: "false", data: { "p_rank" : p_rank, "p_from" : p_from, "p_to" : p_to, "from" : from, "to" : to },
		success: function(rs)
		{
			var rs = $.trim(rs);
			if(rs !="fail" || rs != "")
...
function export_report()
{
	...
	var url = "report/reportController/saleReportController.php?sale_profit_item&export&p_rank="+p_rank+"&p_from="+p_from+"&p_to="+p_to+"&from="+from+"&to="+to+"&token="+token;
	get_download(token);
