<?php
	$pdRange	= isset( $_GET['pdOption'] ) ? $_GET['pdOption'] : 0;
	...
	$qr = "SELECT pd.product_code AS items, ";
	...
	$qr .= "JOIN tbl_product AS pd USING( id_product ) ";
	...
	$qr .= "GROUP BY pod.id_product, pod.id_po";
	
	$qs = dbQuery($qr);
	
	$reportTitle = 'รายงานสินค้าค้างรับ แยกตาม รุ่นสินค้า เลขที่เอกสาร แสดงชื่อผู้จำหน่าย';
	...
	if( dbNumRows($qs) > 0 )
	{
		...
		while( $rs = dbFetchObject($qs) )
		{
			...
			$excel->getActiveSheet()->setCellValue('C'.$row, $rs->items);
	...
	setToken($_GET['token']);
	$file_name = "รายงานสินค้าค้างรับ_".date('Ymd').".xlsx";
