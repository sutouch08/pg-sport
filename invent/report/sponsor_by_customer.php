<?php 
	$page_name = "รายงาน รายการสปอนเซอร์(บุคคลภายนอก)";
...
<script id="report1" type="text/x-handlebars-template">
	...
	<thead>
       ...
       <th style='width:15%;'>ผู้รับ(สโมสร)</th>
...
$("#txt_customer").autocomplete({
	source : "controller/autoComplete.php?get_sponsor_id",
	autoFocus: true,
...
$("#report").click(function(e) {
    ...
    load_in();
	$.ajax({
		url: "report/reportController/sponsorReportController.php?sponsor_by_customer",
		type:"GET", cache:false,
		data:{"customer_rank" : emp, "customer_id" : emp_id, "rank" : rank, "from_date" : from, "to_date" : to, "view" : view},
...
$("#gogo").click(function(e) {
	...
	$("#report_form").attr("action","report/reportController/sponsorReportController.php?export_sponsor_by_customer&customer_rank="+emp+"&customer_id="+emp_id+"&rank="+rank+"&from_date="+from+"&to_date="+to+"&view="+view);
...
function get_detail(id_order)
{
	load_in();
	$.ajax({
		url:"report/reportController/sponsorReportController.php?get_sponsor_detail&id_order="+id_order,
		type:"GET", cache:false,
		success: function(data)
		{
			if(data != 'fail')
