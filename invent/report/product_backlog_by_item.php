...
<table class="table table-striped table-bordered">
	<thead>
    	<tr>
			...
			<th style="width:15%; text-align:center;">รุ่นสินค้า</th>
    ...
    {{#each this}}
    	...
    	<tr style="font-size:12px;">
            ...
            <td align="center"><button type="button" class="btn btn-xs btn-info" onclick="getDetail({{ id_pa }})">รายละเอียด</button></td>
        </tr>
        {{/if}}
    {{/each}}
    </tbody>
...
function getDetail(id_pa)
{
	...
	load_in();
	$.ajax({
		url:"report/reportController/poReportController.php?product_backlog_by_item&detail"	,
		type:"GET", cache:false, data:{"id_pa" : id_pa, "tRange" : tRange, "from" : from, "to" : to },
		success: function(rs)
		{
			load_out();
			var rs = $.trim(rs);
...
function report()
{
	...
	load_in();
	$.ajax({
		url:"report/reportController/poReportController.php?product_backlog_by_item&report",
		type: "GET", cache: "false", data:{"pdRange" : pdRange, "pFrom" : pFrom, "pTo" : pTo, "tRange" : tRange, "from" : from, "to" : to },
		success: function(rs)
		{
			var rs = $.trim(rs);
			if(rs =="nodata")
...
function export_report()
{
	...
	get_download(token);
	var url = "report/reportController/poReportController.php?product_backlog_by_item&export&pdRange="+pdRange+"&pFrom="+pFrom+"&pTo="+pTo+"&tRange="+tRange+"&from="+from+"&to="+to+"&token="+token;
...
function printReport()
{
	...
	var url = "report/reportController/poReportController.php?product_backlog_by_item&printReport&pdRange="+pdRange+"&pFrom="+pFrom+"&pTo="+pTo+"&tRange="+tRange+"&from="+from+"&to="+to;
	printOut(url);
}
...
function printDetail()
{
	...
	var url = "report/reportController/poReportController.php?product_backlog_by_item&printDetail&id_pa="+id_pa+"&tRange="+tRange+"&from="+from+"&to="+to;
	printOut(url);
}
</script>