<?php 
	...
	require "function/order_helper.php";
	 if(isset( $_GET['id_order'] )) : 
			...
			$remark = $rs->comment;
			$qr = dbQuery("SELECT * FROM tbl_order_sponsor WHERE id_order_sponsor = ".$id_order_sponsor);
			$ro = dbFetchArray($qr);
	...
	if( isset($_GET['add']) ){
		...
		$btn = can_do($add, "<a href='index.php?content=order_sponsor' ><button type='button' id='btn_back' class='btn btn-warning'><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button></a>");	
		
	}else if( isset($_GET['edit']) ){
		
		$btn = can_do($edit, "<a href='index.php?content=order_sponsor' ><button type='button' id='btn_back' class='btn btn-warning'><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button></a>");
		...
		$btn = "<a href='index.php?content=order_sponsor' ><button type='button' id='btn_back' class='btn btn-warning' ><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button></a>";
		...
		$btn = can_do($add, "<a href='controller/sponsorController.php?check_add'><button type='button' id='btn_add' class='btn btn-success'><i class='fa fa-plus'></i>&nbsp; เพิ่มใหม่</button></a>");
...
<form action="controller/sponsorController.php?add_order" method="post">
    ...
    <div class="col-lg-4">
    	<div class="input-group">
            ...
            <input type="text" class="form-control" name="customer" id="customer" value="" placeholder="ระบุชื่อสโมสรที่มีสัญญา" autocomplete="off"  required="required" />
        </div>
    </div>
    ...
    <div class="col-lg-4">
    	<div class="input-group">
            ...
            <input type="text" class="form-control" name="employee" id="employee" value="" placeholder="ระบุชื่อผู้เบิก (พนักงาน) " autocomplete="off" required="required"   />
        </div>
    </div>
    <div class="col-lg-5">
    	<div class="input-group">
...
<input type="hidden" id="id_order_sponsor" value="<?php echo $id_order_sponsor; ?>"  />
<input type="hidden" id="save_link" value="controller/sponsorController.php?save_order&id_order=<?php echo $id_order; ?>"  />
	<div class="col-lg-3">
    	<div class="input-group">
        	<span class="input-group-addon">เลขที่เอกสาร</span>
								  ...
								  <div class='modal-dialog' id='modal'>
									<div class='modal-content'>
									  <div class='modal-header'>
										...
										<input type='hidden' name='id_order_sponsor' value="<?php echo $id_order_sponsor; ?>" />
...
<div class="row">               
    <div class="col-lg-12">
        ...
        <dl style='float:left; margin-left:10px;'><dt style='float:left; margin:0px; padding-right:10px'>ผู้ทำรายการ : &nbsp;</dt><dd style='float:left; margin:0px; padding-right:10px'><?php echo employee_name(get_id_user_sponsor($id_order));?></dd> </dt></dl>
        <p class="pull-right">
...
<div class="row">
<div class="col-lg-6">
<form action="controller/sponsorController.php?state_change&id_order=<?php echo $id_order; ?>" method="post">
...
<!-- //*************************************************************** Filter ***************************************************************// -->
<?php
if( isset($_POST['from_date']) && $_POST['from_date'] !=""){ setcookie("sponsor_from_date", date("Y-m-d", strtotime($_POST['from_date'])), time() + 3600, "/"); }
if( isset($_POST['to_date']) && $_POST['to_date'] != ""){ setcookie("sponsor_to_date",  date("Y-m-d", strtotime($_POST['to_date'])), time() + 3600, "/"); }
...
<div class='row'>
	<div class='col-lg-2 col-md-2 col-sm-3 col-sx-3'>
		<div class='input-group'>
			...
			<option value='employee' <?php if( isset($_POST['filter']) && $_POST['filter'] =="employee"){ echo "selected"; }else if( isset($_COOKIE['sponsor_filter']) && $_COOKIE['sponsor_filter'] == "employee"){ echo "selected"; } ?>>ผู้เบิก</option>
			<option value='customer' <?php if( isset($_POST['filter']) && $_POST['filter'] =="customer"){ echo "selected"; }else if( isset($_COOKIE['sponsor_filter']) && $_COOKIE['sponsor_filter'] == "customer"){ echo "selected"; } ?> >ผู้รับ</option>
			<option value='reference' <?php if( isset($_POST['filter']) && $_POST['filter'] =="reference"){ echo "selected"; }else if( isset($_COOKIE['sponsor_filter']) && $_COOKIE['sponsor_filter'] == "reference"){ echo "selected"; } ?>>เอกสาร</option>	
			<option value='not_save' <?php if( isset($_POST['filter']) && $_POST['filter'] =="not_save"){ echo "selected"; }else if( isset($_COOKIE['sponsor_filter']) && $_COOKIE['sponsor_filter'] == "not_save"){ echo "selected"; } ?>>ยังไม่บันทึก</option>
			</select>
		</div>		
	</div>	
	<div class='col-lg-3 col-md-3 col-sm-3 col-sx-3'>
		<div class='input-group'>
			...
			<input class='form-control' type='text' name='search-text' id='search-text' placeholder='ระบุคำค้น' 
                	value="<?php if(isset($_POST['search-text']) && $_POST['search-text'] !=""){ echo $_POST['search-text']; }else if(isset($_COOKIE['sponsor_search-text'])){ echo $_COOKIE['sponsor_search-text']; } ?>" />
			<span class='input-group-btn'><button class='btn btn-default' id='search-btn' type='button'><span id='load'><i class='fa fa-search'></i></span></button></span>
		</div>		
	</div>	
	<div class='col-lg-2 col-md-2 col-sm-2 col-sx-2'>
		<div class='input-group'>
			<span class='input-group-addon'> จาก :</span>
			<input type='text' class='form-control' name='from_date' id='from_date' placeholder='เลือกวัน'  
            	value="<?php if(isset($_POST['from_date']) && $_POST['from_date'] != ""){ 	echo date("d-m-Y", strtotime($_POST['from_date'])); 	}else if( isset($_COOKIE['sponsor_from_date'])){ 	echo date("d-m-Y", strtotime($_COOKIE['sponsor_from_date'])); } ?>" />
		</div>		
	</div>	
	<div class='col-lg-2 col-md-2 col-sm-2 col-sx-2'>
		<div class='input-group'>
			<span class='input-group-addon'>ถึง :</span>
			<input type='test' class='form-control'  name='to_date' id='to_date' placeholder='เลือกวัน' 
            	value="<?php if( isset($_POST['to_date']) && $_POST['to_date'] != "" ){ echo date("d-m-Y", strtotime($_POST['to_date'])); }else if( isset($_COOKIE['sponsor_to_date']) ){ echo date("d-m-Y", strtotime($_COOKIE['sponsor_to_date'])); } ?> " />
		</div>
	</div>
	...
	<div class='col-lg-1 col-md-1 col-sm-1 col-sx-1'>
		<button type='button' class='btn btn-default' onclick="window.location.href='controller/sponsorController.php?clear_filter'"><i class='fa fa-refresh'></i> เคลียร์ฟิลเตอร์</button>
	</div>
</div>
</form>
...
<?php
		$view = "";
		if(isset($_POST['from_date']) && $_POST['from_date'] != ""){$from = date('Y-m-d',strtotime($_POST['from_date'])); }else if( isset($_COOKIE['sponsor_from_date'])){ $from = date('Y-m-d',strtotime($_COOKIE['sponsor_from_date'])); }else{ $from = "";} 
		if(isset($_POST['to_date']) && $_POST['to_date'] != ""){ $to =date('Y-m-d',strtotime($_POST['to_date']));  }else if(  isset($_COOKIE['sponsor_to_date'])){  $to =date('Y-m-d',strtotime($_COOKIE['sponsor_to_date'])); }else{ $to = "";}
		...
		if(isset($_POST['search-text']) && $_POST['search-text'] !="" )
		{
			...
			switch( $_POST['filter']){
				...
				$where = "WHERE id_customer IN($in_cause) AND role = 4 AND order_status = 1 ORDER BY id_order DESC" ; 
				}else{
					$where = "WHERE id_order != NULL AND role = 4";
				}
				break;
				case "employee" :
					$in_cause = "";
					...
					while($i<$rs){
						...
						$where = "WHERE id_employee IN($in) AND role = 4 AND order_status = 1 ORDER BY id_order DESC";
					}else{
						$where = "WHERE id_order != NULL AND role = 4";
					}
				break;
				case "reference" :
				$where = "WHERE reference LIKE'%$text%' AND role = 4 AND order_status = 1 ORDER BY reference";
				break;
				case "not_save" :
				$where = "WHERE role = 4 AND order_status = 0 ORDER BY id_order DESC";
				break;
			}
		}else if(isset($_COOKIE['sponsor_search-text']) && isset($_COOKIE['sponsor_filter'])){
			$text = $_COOKIE['sponsor_search-text'];
			$filter = $_COOKIE['sponsor_filter'];
			switch( $filter){
				case "customer" :
				$in_cause = "";
				$qs = dbQuery("SELECT id_customer FROM tbl_customer WHERE first_name LIKE'%$text%' OR last_name LIKE'%$text%' GROUP BY id_customer");
			...
			$where = "WHERE (date_add BETWEEN '$from' AND '$to') AND role = 4 AND order_status = 1 ORDER BY id_order DESC";
		}
...
<th style="width:8%;">วันที่ปรับปรุง</th>
	...
	<?php if($row > 0) : ?>
        ...
        <?php $link = "style='cursor:pointer;' onclick=\"document.location='index.php?content=order_sponsor&id_order=".$rs['id_order']."&id_order_sponsor=".get_id_order_sponsor($rs['id_order'])."&view_detail=y' \" "; ?>
        <tr style="color:#FFF; font-size:12px; background-color:<?php echo state_color($rs['current_state']); ?>;">  	
            ...
            <td <?php echo $link; ?> ><?php echo customer_name($rs['id_customer']); ?></td>
             <td <?php echo $link; ?> ><?php echo employee_name(get_id_user_sponsor($rs['id_order'])); ?></td>
...
$("#btn_update_order").click(function(e) {
   var id_customer = $("#id_customer").val();
	...
	var id_order_sponsor = $("#id_order_sponsor").val();
	...
	if(!isDate(date)){
		...
		$.ajax({
			url: "controller/sponsorController.php?edit_order=y&id_order="+id_order+"&id_order_sponsor="+id_order_sponsor+"&id_budget="+id_budget,
...
function go_edit(){
	...
	var id_order_sponsor = <?php if(isset($id_order_sponsor)){ echo $id_order_sponsor; }else{ echo "''"; } ?>;
	var target = "index.php?content=order_sponsor&edit=y&id_order="+id_order+"&id_order_sponsor="+id_order_sponsor;
	document.location = target;
}
