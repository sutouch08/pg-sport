...
<!------------------------------------------------  เพิ่มเอกสารใหม่  --------------------------------->
		<!-----------------  หัวเอกสาร  ------------------->
<form id="add_form" action="controller/returnController.php?add=y&role=1" method="post">
<div class="row">
...
<div class="row">
    ...
    <thead>
        ...
        <th>สินค้า</th>
        <th style="width:20%; text-align:center">เลขที่อ้างอิง</th>
...
?>
		<tr id="row<?php echo $rs['id_return_order_detail']; ?>" style="font-size:12px;">
        	<td><?php echo thaiDateTime($rs['date_add']); ?><input type="hidden" class="id_detail" name="id_detail[<?php echo $rs['id_return_order_detail']; ?>]" value="<?php echo $rs['id_return_order_detail']; ?>"  /></td>
            <td id="product<?php echo $rs['id_return_order_detail']; ?>"><?php echo $product_name; ?></td>
            <td align="center">
            <?php if($rs['status']) : ?>
            <?php	echo $rs['order_reference']; ?>
            <?php else : ?>
            <select name="reference[<?php echo $rs['id_return_order_detail']; ?>]" id="reference_<?php echo $rs['id_return_order_detail']; ?>" style="width:100%; font-size:12px;">
			<?php echo  $ro->select_last_item_order($rs['id_product_attribute'], $id_customer);?>
            </select>
            <?php endif; ?>
            </td>
            <td align="right"><?php echo number_format($rs['qty']); ?><input type="hidden" class='qty' id="qty<?php echo $rs['id_return_order_detail'];?>" value="<?php echo $rs['qty']; ?>"/></td>
            ...
            <td align="right"><button type="button" class="btn btn-danger btn-xs btn-block" onclick="delete_row(<?php echo $rs['id_return_order_detail']; ?>)"><i class="fa fa-trash"></i></button></td>
        </tr>
<?php  	endwhile;	?>
<?php else : ?>
...
<div class="row">
    <div class="col-lg-3 col-md-3 col-sm-3">
    	<div class="input-group">
    		<span class="input-group-addon">บาร์โค้ดโซน</span>
        	<input type="text" name="barcode_zone" id="barcode_zone" value="" class="form-control" placeholder="ยิงบาร์โค้โซนเพื่อรับเข้า"  />
        </div>
    </div>
    <div class="col-lg-3 col-md-3 col-sm-3">
    	<div class="input-group">
    ...
    <div class="col-lg-3 col-md-3 col-sm-3">
    	<div class="input-group">
    		<span class="input-group-addon">บาร์โค้ดสินค้า</span>
	        <input type="text" name="barcode_item" id="barcode_item" class="form-control" placeholder="ยิงบาร์โค้ดสินค้าเพื่อรับเข้า" autofocus="autofocus"/>
        </div>
    </div>
	<script>
	document.getElementById('barcode_item').focus();
	</script>
    <div class="col-lg-1 col-md-1 col-sm-1">
    <?php if($edit) : ?>
        <button type="button" id="btn_add_item" class="btn btn-default btn-block" onclick="add_item()"><i class="fa fa-plus"></i>&nbsp; Add</button>
...
?>
		<tr id="row<?php echo $rs['id_return_order_detail']; ?>" style="font-size:12px;">
            ...
            <td align="center">
            <select name="reference[<?php echo $rs['id_return_order_detail']; ?>]" id="reference_<?php echo $rs['id_return_order_detail']; ?>" style="width:100%; font-size:12px;">
			<?php echo  $ro->select_last_item_order($rs['id_product_attribute'], $id_customer, $rs['order_reference'], $rs['id_return_order_detail']);?>
            </select>
            </td>
            <td align="right"><?php echo number_format($rs['qty']); ?><input type="hidden" class='qty' id="qty<?php echo $rs['id_return_order_detail'];?>" value="<?php echo $rs['qty']; ?>"/></td>
            <td id="zone<?php echo $rs['id_return_order_detail']; ?>" align="center"><?php echo get_zone($rs['id_zone']); ?></td>
            <td align="center"><?php echo isActived($rs['status']); ?></td>
            <td align="right"><?php if($edit) : ?><button type="button" class="btn btn-danger btn-xs btn-block" onclick="delete_row(<?php echo $rs['id_return_order_detail']; ?>)"><i class="fa fa-trash"></i></button><?php endif; ?></td>
        </tr>
<?php  	endwhile;	?>
<?php else : ?>
		<tr><td colspan="7" align="center"><h4>---------- ไม่มีรายการ  ----------</h4></td></tr>
<?php endif; ?>
	...
	<?php else : /// if(isset($_GET['id_return_order']) ?>
...
function add()
{
	...
	{
		$.ajax({
			url:"controller/returnController.php?check_order",
				...
				if(rs == "ok")
				{
					$.ajax({
						...
						success: function(res)
						{
							if($.trim(res) != "fail")
							{
								window.location.href="index.php?content=order_return&add=y&id_return_order="+res;
								load_out();
							}else{
								load_out();
...
function update(id)
{
	...
	$.ajax({
		...
		data:{ "id_return_order" : id, "remark" : remark },
		success: function(ra)
		{
			if(ra == "success")
			{
...
$("#order_reference").keyup(function(e) {
    var id_customer = $("#id_customer").val();
...
function valid_qty()
{
	...
	$(".id_detail").each(function(index, element) {
		var id	= $(this).val();
        var o_qty = $("#reference_"+id).val();
		var arr = o_qty.split(" : ");
		var ref_qty = parseInt(arr[1]);
