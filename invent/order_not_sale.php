<?php 
	...
	/*function orderStateList($id_role){
		...
		if($_GET['content'] == "order_not_sale2"){
			$id_role = 2;
		}else if($_GET['content'] == "order_not_sale3"){
			$id_role = 3;
		}else if($_GET['content'] == "order_not_sale4"){
			$id_role = 4;
    ...
    <div class="col-sm-6">
	   ...
	   if(isset($_GET['edit'])&&isset($_GET['id_order'])){
		   ...
		   if($order->valid==1 || $order->current_state !=1 && $order->current_state !=3){ $active = "style='display:none;'";}else{$active = ""; }
		    echo"
		   <li><a href='index.php?content=order_not_sale$id_role' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link'><span class='glyphicon glyphicon-arrow-left' style='color:#5cb85c; font-size:30px;'></span><br />กลับ</button></a></li>
       		<li $can_edit ><a href='index.php?content=order_not_sale$id_role&add=y&id_order=$id_order' style='text-align:center; background-color:transparent;' $active ><button type='button' class='btn btn-link'  $active><span class='glyphicon glyphicon-pencil' style='color:#5cb85c; font-size:30px;'></span><br />แก้ไข</button></a></li>";
		   ...
		   echo"
		   <li $can_add><a href='index.php?content=order_not_sale$id_role&add=y' style='text-align:center; background-color:transparent; padding-bottom:0px;' ><button type='button' class='btn btn-link' ><span class='glyphicon glyphicon-plus-sign' style='color:#5cb85c; font-size:30px;'></span><br />$page_name</button></a></li>";
	   }
	   ?>
       </ul>
    </div>
