<?php
class return_support
{
public $id_return_support;
public $reference;
...
public function __construct($id = "")
{
	if($id != "" )
	{
		$qs = dbQuery("SELECT * FROM tbl_return_support WHERE id_return_support = ".$id);
		if(dbNumRows($qs) == 1 )
		{
			$rs = dbFetchArray($qs);
			$this->id_return_support	= $id;
...
public function update($id_return_support, array $data)
{
	$res = false;
	$this->get_data($id_return_support);
	$total_amount 	= $this->total_return_amount($this->reference) * (-1);  ///  ยอดเงินรวมใน order_detail_sold * -1 เพื่อทำให้เป็น บวก
	$id_budget		= $this->get_id_budget_by_id_order($this->get_id_order_by_reference($this->order_reference, 7));
	$qm				= dbQuery("UPDATE tbl_order_detail_sold SET id_employee = ".$data['id_employee']." WHERE reference = '".$this->reference."'");
	if($qm)
	{
		$qr	= dbQuery("UPDATE tbl_support_budget SET balance = balance - ".$total_amount." WHERE id_support_budget = ".$id_budget);
		if($qr)
		{
			$new_id_budget 	= $this->get_id_budget_by_id_order($this->get_id_order_by_reference($data['order_reference'], 7));
			$qa	= dbQuery("UPDATE tbl_support_budget SET balance = balance + ".$total_amount." WHERE id_support_budget = ".$new_id_budget);
			if($qa)
			{
...
public function drop_all_detail($id_return_support)
{
	$res	= true;
	$qs	= dbQuery("SELECT id_return_support_detail FROM tbl_return_support_detail WHERE id_return_support = ".$id_return_support);
	if(dbNumRows($qs) > 0 )
	{
		while($rs = dbFetchArray($qs) )
		{
			$rd = $this->drop_detail($rs['id_return_support_detail']);
			if(!$rd){ $res		= false; }
		}
	}
	return $res;
...
public function add_item($id_return_support, array $data)
{
	...
	$status						= $data['status'];
	$qs = dbQuery("SELECT qty FROM tbl_return_support_detail WHERE id_product_attribute = ".$id_product_attribute." AND id_return_support = ".$id_return_support." AND id_zone = ".$id_zone." AND status= 0");
	if(dbNumRows($qs) == 1 )
	{
	...
	if($qr)
	{
		$qa = dbQuery("SELECT * FROM tbl_return_support_detail WHERE id_return_support = ".$id_return_support." AND id_product_attribute = ".$id_product_attribute." AND id_zone = ".$id_zone." AND status = 0");
		$rs = dbFetchArray($qa);
		$product		= new product();
...
{
	$qty 	= 0;
	$qs 	= dbQuery("SELECT SUM(qty) AS qty FROM tbl_return_support_detail WHERE id_return_support = ".$id_return_support);
	if(dbNumRows($qs) == 1 )
	{
		$rs 	= dbFetchArray($qs);
		$qty 	= $rs['qty'];
...
{
	$qs = dbQuery("SELECT * FROM tbl_return_support_detail WHERE id_return_support = ".$id_return_support." AND status = 0");
	return $qs;
}
...
{
	$qs = dbQuery("UPDATE tbl_return_support_detail SET status = ".$status." WHERE id_return_support_detail = ".$id_return_support_detail);
	return $qs;
}
...
{
	$result = true;
	$qs = $this->return_detail($this->id_return_support);
