...
function stock_movement($move_type, $reason, $id_product_attribute,$id_warehouse, $qty, $reference,$date_upd, $id_zone=0){
	if($move_type=="in"){
		$sql = dbQuery("SELECT id_stock_movement, move_in FROM tbl_stock_movement WHERE id_product_attribute = ".$id_product_attribute." AND reference = '".$reference."' AND id_zone = ".$id_zone);
		$row = dbNumRows($sql);
		if($row == 1){
			$rs = dbFetchArray($sql);
			$move_in = $rs['move_in'] + $qty;
			$qr = dbQuery("UPDATE tbl_stock_movement SET move_in = ".$move_in." WHERE id_stock_movement = ".$rs['id_stock_movement']);
		}else{
			$qr = dbQuery("INSERT INTO tbl_stock_movement (id_reason, id_product_attribute,id_warehouse, move_in, reference, date_upd, id_zone) VALUES ($reason, $id_product_attribute,$id_warehouse, $qty, '$reference', '$date_upd', $id_zone)");
		}	
	}else if($move_type=="out"){
		$sql = dbQuery("SELECT id_stock_movement, move_out FROM tbl_stock_movement WHERE id_product_attribute = ".$id_product_attribute." AND reference = '".$reference."' AND id_zone = ".$id_zone);
		$row = dbNumRows($sql);
		if($row == 1){
			$rs = dbFetchArray($sql);
			$move_out = $rs['move_out'] + $qty;
			$qr = dbQuery("UPDATE tbl_stock_movement SET move_out = ".$move_out." WHERE id_stock_movement = ".$rs['id_stock_movement']);
		}else{
			$qr = dbQuery("INSERT INTO tbl_stock_movement (id_reason, id_product_attribute,id_warehouse, move_out, reference, date_upd, id_zone) VALUES ($reason, $id_product_attribute,$id_warehouse, $qty, '$reference', '$date_upd', $id_zone)");
		}
	}
	if($qr){ return true; }else{ return false; }
}
