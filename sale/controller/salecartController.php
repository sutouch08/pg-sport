<?php 
	...
	if(isset($_GET['updateproductcartfull'])){
		...
		$id_product_attribute = $_GET['id_product_attribute'];
		$qty = $_GET['qty'];
		list($qty_stock) = dbFetchArray(dbQuery("select qty from stock_qty where id_product_attribute = '$id_product_attribute'"));
	...
	if(isset($_GET['confirm'])){
		if(isset($_COOKIE['user_id'])){ $user_id = $_COOKIE['user_id'];}else{ $user_id = 0;}
		$product = new product();		
		...
		if($id_payment == "3"){
			...
			if($to_date > "$date_dew"){
				$message = "ลูกค้ารายนี้มีสินค้าาที่เกินกำหนดชำระ กรุณาติดต่อเจ้าหน้าทีบัญชีเพื่อดำเนินการชำระเงิน";
				header("location: ../index.php?content=cart&error=$message");
			}else if($credit_balance < "0"){
				$message = "ลูกค้ารายนี้มีวงเงินเครดิตไม่พอสำหรับสั่งออร์เดอร์นี้ กรุณาติดต่อเจ้าหน้าที่บัญชีเพื่อดำเนินการชำระเงิน";
				header("location: ../index.php?content=cart&error=$message");
			}else{
				$true = "Y";
			}
		...
		$valid = 0;
		$reference = get_max_reference("PREFIX_ORDER","tbl_order","reference");
		...
		dbQuery("INSERT INTO tbl_order_state_change (id_order, id_order_state, id_employee, date_add) VALUES ($id_order,1,$user_id,NOW())");
		$sql = dbQuery("select id_cart_product,id_product_attribute,qty from tbl_cart_product where id_cart = '$id_cart'");
						...
						while($i<$row){
							...
							//$total_amount= $product_qty * $product->product_price;
							//พรุ้งนี้ทำตรงนี้ต่อ
							if($product->discount_type == "percentage"){
								...
								$reduction_parcent = $product->Discount1($id_product,$id_customer);
								$reduction_amount = 0;
	...
	//if(isset($_GET['test'])){
		//setcookie("id_customer","2",time()+(3600*24*30));//Expire in 30 days
		//setcookie("customer_name","ศุภภาสิทธิ์"."&nbps"."อินทร์เพ็ญ",time()+(3600*24*30));
		//header("location: ../index.php?content=cart");
		//echo $_COOKIE['cus
	//}
	//echo $company->default_active;
